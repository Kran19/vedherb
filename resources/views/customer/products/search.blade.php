...
@section('title', 'Search Results for "' . ($search ?? '') . '" - Ved Herbs & Ayurveda')
        ...
        <div class="max-w-7xl mx-auto px-4 text-center">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="text-stone-800 font-semibold text-sm">Search Results</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="animate-fade-in-up">
                <h1 class="text-3xl md:text-4xl font-bold text-gray-800 mb-4">
                    Search results for: <span class="text-emerald-700">"{{ $search ?? '' }}"</span>
                </h1>
                <p class="text-gray-600">
                    We found {{ $paginator['total'] ?? 0 }} products matching your search.
                </p>
            </div>
        </div>
    </div>
            ...
            <div id="filterSidebar" class="mobile-filter-drawer lg:static lg:block lg:w-1/4 lg:h-auto lg:bg-transparent lg:shadow-none lg:transform-none lg:z-auto">
                <div class="bg-white lg:rounded-xl lg:shadow p-5 sticky top-6 filter-section h-full lg:h-auto overflow-y-auto lg:overflow-visible">
                    ...
                    <div class="flex justify-between items-center mb-5">
                        ...
                        @if (request()->hasAny(['search', 'min_price', 'max_price', 'category_id', 'brand_id', 'in_stock']))
                            <a href="{{ url()->current() }}"
                    ...
                    <form id="filterForm" method="GET" action="{{ url()->current() }}">
                        <!-- Search Box (Keep in sidebar too) -->
                        ...
                        @if (isset($filters['categories']) && count($filters['categories']) > 0)
                            <div id="filter-categories" class="mb-5">
                                <h4 class="font-semibold text-gray-800 mb-3 text-sm">Categories</h4>
                                <div class="space-y-1 max-h-60 overflow-y-auto">
                                    @foreach ($filters['categories'] as $cat)
                                        <label class="flex items-center justify-between p-2 hover:bg-emerald-50 rounded-lg cursor-pointer transition-colors">
                                            <div class="flex items-center">
                                                ...
                                                <span class="text-gray-700 ml-3 text-sm">{{ $cat['name'] }}</span>
                                            </div>
                     ...
                     <!-- Special Filters -->
                     <div id="filter-special" class="space-y-3 pt-5 border-t">
                        <h4 class="font-semibold text-gray-800 mb-2">Special Collections</h4>
                        <div class="grid grid-cols-2 gap-2">
                            <a href="{{ request()->fullUrlWithQuery(['is_featured' => 1]) }}"
                                class="text-center py-2 px-3 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 text-xs">
                                <i class="fas fa-star mr-1"></i> Featured
                            </a>
                            <a href="{{ request()->fullUrlWithQuery(['is_new' => 1]) }}"
                                class="text-center py-2 px-3 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 text-xs">
                                <i class="fas fa-bolt mr-1"></i> New arrivals
                            </a>
                            <a href="{{ request()->fullUrlWithQuery(['is_bestseller' => 1]) }}"
                                class="text-center py-2 px-3 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 text-xs">
                                <i class="fas fa-fire mr-1"></i> Best sellers
                            </a>
                            <a href="{{ request()->fullUrlWithQuery(['has_discount' => 1]) }}"
                                class="text-center py-2 px-3 bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-200 text-xs">
            ...
            <div class="lg:w-3/4">
                    ...
                    <div>
                        <p class="text-gray-700">
                            @if (isset($paginator['total']) && $paginator['total'] > 0)
                                ...
                                <span class="font-semibold">{{ $paginator['total'] ?? 0 }}</span> search results
                            @else
                                No products found
                            @endif
                        </p>
                ...
                @else
                    <div class="text-center py-16 bg-white rounded-xl shadow">
                        ...
                        <p class="text-gray-600 mb-6">Sorry, we couldn't find any products matching your search.</p>
