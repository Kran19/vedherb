...
@section('content')
                ...
                <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label for="product_code" class="block text-sm font-medium text-gray-700 mb-1">Product Code
                                    (Art. No.)</label>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100" id="media-section">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="hidden" name="main_image_id" id="main_image_id"
                                value="{{ old('main_image_id', $product->main_image_id) }}">
                ...
                @if($product->product_type === 'simple')
                    <div id="simple-product-fields" class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}"
                                        step="0.01" min="0"
                                        class="w-full pl-8 pr-4 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500">
                                </div>
                            ...
                            <div>
                                <label for="compare_price" class="block text-sm font-medium text-gray-700 mb-1">Compare at
                                    Price</label>
                ...
                @if($product->product_type === 'configurable')
                    <div id="configurable-product-fields" class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 border" id="variants-table">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Variant</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase w-32">SKU
                                        </th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200" id="variants-container">
                                    ...
                                    @foreach($product->variants as $idx => $variant)
                                        <tr id="variant-row-{{ $idx }}">
                                            ...
                                            <td class="px-3 py-2">
                                                <input type="number" name="variants[{{ $idx }}][stock_quantity]"
                                                    value="{{ $variant->stock_quantity }}"
                                                    class="w-full px-2 py-1 border rounded text-sm">
                                            </td>
                                            <td class="px-3 py-2">
                                                <div id="variant-images-{{ $idx }}" class="flex gap-2 flex-wrap items-center">
                                                    {{-- Main Image --}}
                                                    @if($variant->primaryImage && $variant->primaryImage->media)
                                                        <div
                                                            class="relative w-12 h-12 variant-main-thumb border-2 border-blue-500 rounded overflow-hidden group">
                                                            <img src="{{ asset('storage/' . $variant->primaryImage->media->file_path) }}"
                                                                class="w-full h-full object-cover">
                                                            <button type="button" onclick="removeVariantImage({{ $idx }}, 'main', this)"
                                                                class="absolute top-0 right-0 bg-red-500 text-white p-0.5 opacity-0 group-hover:opacity-100 transition rounded-bl">
                                                    ...
                                                    @else
                                                        <div
                                                            class="relative w-12 h-12 variant-main-thumb border-2 border-dashed border-gray-300 rounded flex items-center justify-center bg-gray-50 text-xs text-gray-400">
                                                    ...
                                                    @foreach($variant->images as $vImg)
                                                        @if(!$variant->primaryImage || $vImg->id !== $variant->primaryImage->media_id)
                                                            <div
                                                                class="relative w-12 h-12 border border-gray-200 rounded overflow-hidden group">
                                                                <img src="{{ asset('storage/' . $vImg->file_path) }}"
                                                                    class="w-full h-full object-cover">
                                                                <button type="button"
                                                                    onclick="removeVariantImage({{ $idx }}, 'gallery', this, {{ $vImg->id }})"
                                                                    class="absolute top-0 right-0 bg-red-500 text-white p-0.5 opacity-0 group-hover:opacity-100 transition rounded-bl">
                                                                    <svg class="w-3 h-3" fill="none" stroke="currentColor"
                                                                        viewBox="0 0 24 24">
                                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                ...
                                                <button type="button" onclick="openVariantMediaModal({{ $idx }})"
                                                    class="text-xs text-blue-600 hover:text-blue-800 mt-2 block w-full text-center">Manage
                                                    Images</button>

                                                {{-- Hidden Inputs for Images --}}
                                                <input type="hidden" name="variants[{{ $idx }}][main_image_id]"
                                                    id="variant-main-input-{{ $idx }}"
                                                    value="{{ ($variant->primaryImage && $variant->primaryImage->media) ? $variant->primaryImage->media_id : '' }}">
                                                <div id="variant-gallery-inputs-{{ $idx }}">
                                                    @foreach($variant->images as $vImg)
                                                        @if(!$variant->primaryImage || $vImg->id !== $variant->primaryImage->media_id)
                                                            <input type="hidden" name="variants[{{ $idx }}][gallery_image_ids][]"
                                                                value="{{ $vImg->id }}">
                                                        @endif
                                                    @endforeach
                                                </div>
                                            ...
                                            <td class="px-3 py-2 text-center">
                                                <input type="radio" name="default_variant_index" value="{{ $idx }}" {{ $variant->is_default ? 'checked' : '' }}
                                                    onclick="document.querySelectorAll('.is-default-input').forEach(el => el.value=0); document.getElementById('is-default-{{ $idx }}').value=1;">
                                                <input type="hidden" id="is-default-{{ $idx }}"
                                                    name="variants[{{ $idx }}][is_default]"
                                                    value="{{ $variant->is_default ? '1' : '0' }}" class="is-default-input">
                                            </td>
                                            <td class="px-3 py-2 text-center">
                                                <button type="button"
                                                    onclick="document.getElementById('variant-row-{{ $idx }}').remove()"
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center space-x-2">
                            <input type="checkbox" name="is_new" id="is_new" value="1" {{ old('is_new', $product->is_new) ? 'checked' : '' }} class="rounded text-blue-500 focus:ring-blue-500 h-4 w-4">
                ...
                <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" value="{{ ucfirst($product->product_type) }}" disabled
                                class="w-full px-4 py-2 border border-gray-200 bg-gray-100 rounded-lg text-gray-500 cursor-not-allowed">
                            <input type="hidden" name="product_type" value="{{ $product->product_type }}">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Main Category</label>
                            <input type="text" value="{{ $product->mainCategory ? $product->mainCategory->name : 'None' }}"
                                disabled
                                class="w-full px-4 py-2 border border-gray-200 bg-gray-100 rounded-lg text-gray-500 cursor-not-allowed">
                        ...
                        <div>
                            ...
                            <select name="tag_ids[]" id="tag_ids" multiple
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 h-32">
                                @php
                            ...
                            <p class="text-xs text-gray-500 mt-1">Hold Ctrl (Windows) or Cmd (Mac) to select multiple tags.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <div>
                                ...
                                <input type="number" name="length" value="{{ old('length', $product->length) }}"
                                    placeholder="cm" class="w-full px-2 py-1 border rounded">
                            </div>
                            <div>
    ...
    <div id="media-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="flex flex-col md:flex-row justify-between mb-4 space-y-2 md:space-y-0">
                        ...
                        <div class="flex items-center space-x-2">
                            <label
                                class="cursor-pointer bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded shadow transition">
                    ...
                    <div id="media-grid"
                        class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 max-h-96 overflow-y-auto p-2 border rounded">
                        <!-- Loaded dynamically -->
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="button" id="media-select-btn"
...
@push('scripts')
    <script>
        // Variant Image Removal
        function removeVariantImage(idx, type, btn, id = null) {
            if (type === 'main') {
                ...
                const input = inputContainer.querySelector(`input[value="${id}"]`);
        ...
        document.getElementById('add-variant-btn')?.addEventListener('click', function () {
            ...
            const tableBody = document.getElementById('variants-container');
            const newIdx = 'new_' + Date.now();
            const tr = document.createElement('tr');
            tr.id = `variant-row-${newIdx}`;
        ...
        function generateSkuForVariant(idx) {
            const skuInput = document.querySelector(`input[name="variants[${idx}][sku]"]`);
            const baseSku = document.getElementById('product_code').value || 'PROD';
            const timestamp = Math.floor(Date.now() / 1000).toString().substr(-4);
            skuInput.value = `${baseSku}-VAR-${timestamp}`;
        }
